...
The symbol `MBEDTLS_PSA_CRYPTO_CONFIG` in `mbedtls/mbedtls_config_defines.h` determines whether `psa/crypto_config.h` is used.
...
When `MBEDTLS_PSA_CRYPTO_CONFIG` is set, the header file `mbedtls/mbedtls_config_defines.h` needs to define all the `MBEDTLS_xxx_C` configuration symbols, including the ones deduced from the PSA Crypto configuration. It does this by including the new header file **`mbedtls/config_psa.h`**, which defines the `MBEDTLS_PSA_BUILTIN_xxx` symbols and deduces the corresponding `MBEDTLS_xxx_C` (and other) symbols.
...
When `MBEDTLS_PSA_CRYPTO_CONFIG` is not set, the configuration of Mbed TLS works as before, and the inclusion of non-PSA code only depends on `MBEDTLS_xxx` symbols defined (or not) in `mbedtls/mbedtls_config_defines.h`. Furthermore, the new header file **`mbedtls/config_psa.h`** deduces PSA configuration symbols (`PSA_WANT_xxx`, `MBEDTLS_PSA_BUILTIN_xxx`) from classic configuration symbols (`MBEDTLS_xxx`).
...
Since some existing applications use a handwritten `mbedtls/mbedtls_config_defines.h` or an edited copy of `mbedtls/mbedtls_config_defines.h` from an earlier version of Mbed TLS, `mbedtls/config_psa.h` must be included via an already existing header that is not `mbedtls/mbedtls_config_defines.h`, so it is included via `psa/crypto.h` (for example from `psa/crypto_platform.h`).
...
| ------------------------- | ----------------------------------------- | -------------------------------------- |
| `MBEDTLS_xxx_C`           | `mbedtls/mbedtls_config_defines.h` (U) or | `mbedtls/mbedtls_config_defines.h` (U) |

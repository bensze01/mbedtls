...
# * Run `make clean`.
# * Restore `include/mbedtks/mbedtls_config_defines.h` from a backup made before running
#   the component.
...
pre_check_git () {
    if [ $FORCE -eq 1 ]; then
        ...
        if ! git diff --quiet include/mbedtls/mbedtls_config_defines.h; then
            err_msg "Warning - the configuration file 'include/mbedtls/mbedtls_config_defines.h' has been edited. "

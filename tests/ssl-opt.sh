...
guess_config_name() {
    if git diff --quiet ../include/mbedtls/mbedtls_config_defines.h 2>/dev/null; then
        echo "default"
    else
        echo "unknown"
    fi
...
CONFIG_H='../include/mbedtls/mbedtls_config_defines.h'
...
# Read boolean configuration options from mbedtls_config_defines.h for easy and quick
...
# skip next test if the flag is not enabled in mbedtls_config_defines.h
...
# skip next test if the flag is enabled in mbedtls_config_defines.h
...
# algorithm stronger than SHA-1 is enabled in mbedtls_config_defines.h
requires_config_enabled MBEDTLS_SSL_RENEGOTIATION
